...
`;function ve(t){let e=document.getElementById("fatal-error");e||(e=document.createElement("div"),e.id="fatal-error",Object.assign(e.style,{position:"fixed",left:"16px",right:"16px",bottom:"16px",padding:"14px 16px",borderRadius:"16px",border:"1px solid rgba(255, 110, 110, 0.35)",background:"rgba(40, 8, 12, 0.9)",color:"#ffd7d7",fontFamily:"Consolas, monospace",fontSize:"12px",lineHeight:"1.5",zIndex:"10000",whiteSpace:"pre-wrap"}),document.body.appendChild(e)),e.textContent=t}window.addEventListener("error",t=>{t.message&&ve(t.message)});window.addEventListener("unhandledrejection",t=>{const e=t.reason?.message||String(t.reason||"Unknown promise rejection");ve(e)});function lt(t=768){const e=Math.min(window.devicePixelRatio||1,1.2),n=Math.max(window.innerWidth,window.innerHeight),i=Math.min(1,t/n)*e;return{width:Math.max(320,Math.round(window.innerWidth*i)),height:Math.max(320,Math.round(window.innerHeight*i))}}function ue(t){return new E().setRGB(t[0],t[1],t[2])}const g=document.getElementById("canvas"),T=new ke({canvas:g,antialias:!0,alpha:!0});T.setPixelRatio(Math.min(window.devicePixelRatio,2));T.setSize(window.innerWidth,window.innerHeight);T.toneMapping=De;T.toneMappingExposure=1.08;T.outputColorSpace=Le;const N=document.createElement("canvas"),{width:M,height:F}=lt();N.width=M;N.height=F;const H=N.getContext("webgl2",{alpha:!0,premultipliedAlpha:!1});if(!H)throw new Error("WebGL2 is required for the ink demo.");const _=new Je(H,M,F);await _.init({vert:Ke,advect:et,div:tt,pres:nt,sub:it,render:ot,splat:rt});const R=new Ie(N);R.minFilter=de;R.magFilter=de;R.generateMipmaps=!1;R.wrapS=fe;R.wrapT=fe;const x=new Ue;x.background=new E(329227);x.fog=new We(329227,.055);const A=new ze(48,window.innerWidth/window.innerHeight,.1,100),O=new Oe(0,1.45,0),h={theta:.46,phi:1.02,radius:10.4};let f=null;function te(){A.position.set(O.x+h.radius*Math.sin(h.phi)*Math.sin(h.theta),O.y+h.radius*Math.cos(h.phi),O.z+h.radius*Math.sin(h.phi)*Math.cos(h.theta)),A.lookAt(O),f?.u_cameraPos.value.copy(A.position)}te();const $=new He;let ne=26,q="sheet",V="paint",L="ink",S=0;const l={surfaceHeight:.17,surfaceRelief:.055,meniscus:.06,absorption:1.75,refraction:.22,fresnel:1.05,specular:1.15,caustics:.42},d={paintForce:.28,paintAmount:1,bubbleForce:.42,bubbleCarve:.95,ambientMotion:.14};x.add(new Ve(8162214,1119002,1.25));const k=new Xe(15134975,1.22);k.position.set(-4,7,5);x.add(k);const I=new pe(8038399,1.8,18,2);I.position.set(3.5,2.8,-2.2);x.add(I);const ie=new pe(6287615,1.2,14,2);ie.position.set(-2.8,1.8,3);x.add(ie);const oe=new y(new Ge(18,64),new Z({color:855830,roughness:.96,metalness:.04}));oe.rotation.x=-Math.PI/2;oe.position.y=-.01;x.add(oe);const w=new Ne;x.add(w);const ut=new Z({color:1514023,roughness:.34,metalness:.46,emissive:592660,emissiveIntensity:.8}),ct=new J({color:856347,roughness:.18,metalness:.08,transmission:.18,thickness:.8,clearcoat:1,clearcoatRoughness:.12}),_e=new y(new U(6.8,.72,6.8),ut);_e.position.y=.34;w.add(_e);const ge=new y(new U(6,.18,6),ct);ge.position.y=.82;w.add(ge);const dt=new Z({color:2764613,roughness:.3,metalness:.56,emissive:790560,emissiveIntensity:.9});[{x:0,y:.99,z:2.76,sx:5.62,sy:.24,sz:.16},{x:0,y:.99,z:-2.76,sx:5.62,sy:.24,sz:.16},{x:2.76,y:.99,z:0,sx:.16,sy:.24,sz:5.62},{x:-2.76,y:.99,z:0,sx:.16,sy:.24,sz:5.62}].forEach(t=>{const e=new y(new U(t.sx,t.sy,t.sz),dt);e.position.set(t.x,t.y,t.z),w.add(e)});const p=5.32,j=1.06,xe=new J({color:1120804,roughness:.08,metalness:.04,transmission:.58,thickness:1.25,clearcoat:1,clearcoatRoughness:.08,transparent:!0,opacity:.88,ior:1.22}),be=new y(new U(p+.18,.08,p+.18),xe);be.position.y=.99;w.add(be);const ye=new J({color:1582132,roughness:.05,metalness:.02,transmission:.74,thickness:.82,clearcoat:1,clearcoatRoughness:.05,transparent:!0,opacity:.82,ior:1.24});[{x:0,y:1.155,z:p*.5+.06,sx:p+.1,sy:.34,sz:.08},{x:0,y:1.155,z:-2.72,sx:p+.1,sy:.34,sz:.08},{x:p*.5+.06,y:1.155,z:0,sx:.08,sy:.34,sz:p+.1},{x:-2.72,y:1.155,z:0,sx:.08,sy:.34,sz:p+.1}].forEach(t=>{const e=new y(new U(t.sx,t.sy,t.sz),ye);e.position.set(t.x,t.y,t.z),w.add(e)});const we=new me(p,p,180,180);we.rotateX(-Math.PI/2);const Y=new me(p,p);Y.rotateX(-Math.PI/2);f={u_inkMap:{value:R},u_texel:{value:new K(1/M,1/F)},u_domainSize:{value:p},u_cameraPos:{value:A.position.clone()},u_surfaceHeight:{value:l.surfaceHeight},u_surfaceRelief:{value:l.surfaceRelief},u_meniscus:{value:l.meniscus},u_absorption:{value:l.absorption},u_refraction:{value:l.refraction},u_fresnel:{value:l.fresnel},u_specular:{value:l.specular},u_caustics:{value:l.caustics},u_time:{value:0},u_viewMode:{value:0},u_lightDir:{value:k.position.clone().normalize()},u_lightColor:{value:k.color.clone().multiplyScalar(k.intensity)},u_rimColor:{value:new E(10282495)},u_shadowColor:{value:new E(528408)}};const ft=new qe({name:"InkFlowSurface",uniforms:f,vertexShader:st,fragmentShader:at,transparent:!0,depthWrite:!1,side:ee,glslVersion:je,defines:{USE_UV:""}}),re=new y(we,ft);re.position.y=j;re.renderOrder=3;w.add(re);const P=new y(Y,new G({map:R,transparent:!0,depthWrite:!1,opacity:.22,blending:Ye,side:ee,color:726820}));P.position.y=j-.05;P.scale.setScalar(.992);P.renderOrder=1;w.add(P);const B=new y(Y,new G({map:R,transparent:!0,depthWrite:!1,opacity:.14,blending:he,side:ee,color:14874111}));B.position.y=j-.075;B.scale.setScalar(.968);B.renderOrder=0;w.add(B);const X=new y(Y,new G({color:16777215,transparent:!0,opacity:0,depthWrite:!1}));X.position.y=j+.015;w.add(X);const W=new y(new $e(4.6,.03,16,80),new G({color:3163500,transparent:!0,opacity:.22,blending:he,depthWrite:!1}));W.rotation.x=Math.PI/2;W.position.y=.42;x.add(W);const se=[{id:"lagoon",label:"Lagoon",ink:[.1,.72,.95],edge:[.86,.97,1],bgA:[.02,.08,.14],bgB:[0,.01,.03],accent:5822463},{id:"ember",label:"Ember",ink:[.94,.42,.15],edge:[1,.87,.68],bgA:[.11,.04,.03],bgB:[.01,0,.02],accent:16747595},{id:"indigo",label:"Indigo",ink:[.47,.56,1],edge:[.83,.88,1],bgA:[.05,.04,.12],bgB:[.01,.01,.03],accent:8360191},{id:"mono",label:"Mono",ink:[.82,.87,.92],edge:[1,1,1],bgA:[.08,.08,.09],bgB:[.01,.01,.02],accent:15068921}];let Ee=se[0];function ae(){const t=L==="ink";f.u_viewMode.value=t?0:1,P.material.opacity=t?.18+l.caustics*.14:.08,B.material.opacity=t?.04+l.caustics*.24:.03}function pt(t){const e=new E(t.accent),n=ue(t.bgA).lerp(new E(397336),.45);f.u_rimColor.value.copy(ue(t.edge).lerp(e,.22)),f.u_shadowColor.value.copy(n),P.material.color.copy(e.clone().multiplyScalar(.35).lerp(new E(529179),.68)),B.material.color.copy(e.clone().lerp(new E(16777215),.42)),xe.color.copy(n.clone().lerp(e,.08)),ye.color.copy(n.clone().lerp(e,.16).lerp(new E(16777215),.08)),I.color.copy(e),ie.color.copy(e.clone().lerp(new E(16777215),.32)),W.material.color.copy(e.clone().multiplyScalar(.55))}function mt(t,e){t==="gravity"&&(_.gravityY=-e),t==="dt"&&(_.dt=e),t==="jacobiIter"&&(_.jacobiIter=Math.round(e)),t==="brushRadius"&&(ne=e),t==="paintForce"&&(d.paintForce=e),t==="paintAmount"&&(d.paintAmount=e),t==="bubbleForce"&&(d.bubbleForce=e),t==="bubbleCarve"&&(d.bubbleCarve=e),t==="ambientMotion"&&(d.ambientMotion=e),t==="velocityDamping"&&(_.velocityDamping=e),t==="inkRetention"&&(_.inkRetention=e),t==="surfaceHeight"&&(l.surfaceHeight=e,f.u_surfaceHeight.value=e),t==="surfaceRelief"&&(l.surfaceRelief=e,f.u_surfaceRelief.value=e),t==="meniscus"&&(l.meniscus=e,f.u_meniscus.value=e),t==="absorption"&&(l.absorption=e,f.u_absorption.value=e),t==="refraction"&&(l.refraction=e,f.u_refraction.value=e),t==="fresnel"&&(l.fresnel=e,f.u_fresnel.value=e),t==="specular"&&(l.specular=e,f.u_specular.value=e),t==="caustics"&&(l.caustics=e,f.u_caustics.value=e,ae()),t==="exposure"&&(T.toneMappingExposure=e),t==="fogDensity"&&(x.fog.density=e)}const u={simulation:{section:"Simulation",sectionOrder:1},brush:{section:"Brush & Flow",sectionOrder:2},surface:{section:"Surface Form",sectionOrder:3},optics:{section:"Optical Look",sectionOrder:4}};new Qe({gravity:{...u.simulation,order:1,label:"Gravity",min:0,max:20,step:.5,value:9.8,description:"液体を下方向へ引く強さです。大きいほど滴下や崩れが速くなり、壁際まで押し出された流れの戻りも強くなります。"},dt:{...u.simulation,order:2,label:"Time Step",min:.01,max:.35,step:.01,value:.12,description:"1 step ごとの進み量です。大きいほど変化は速く見えますが、値を上げすぎると表面が荒れて不安定になります。"},jacobiIter:{...u.simulation,order:3,label:"Pressure Iter",min:5,max:60,step:1,value:28,decimals:0,description:"圧力解法の反復回数です。増やすほど体積保持と流線が安定し、少ないとにじみや圧縮感が残ります。"},velocityDamping:{...u.simulation,order:4,label:"Velocity Damping",min:.985,max:1,step:5e-4,value:.9985,description:"流速の減衰量です。下げると流れが早く失速し、上げると尾や巻き返しが長く残ります。"},inkRetention:{...u.simulation,order:5,label:"Ink Retention",min:.96,max:1,step:.001,value:1,description:"インク量の保持率です。下げると薄く消えていき、上げると濃い塊や筋が長く残ります。"},brushRadius:{...u.brush,order:1,label:"Brush Radius",min:10,max:60,step:1,value:26,decimals:0,description:"クリックやドラッグで加える範囲です。大きいほど大きなインク塊になり、広い膜や波打つ境界が出やすくなります。"},paintForce:{...u.brush,order:2,label:"Paint Force",min:.05,max:.8,step:.01,value:d.paintForce,description:"ペイント時に流れへ与える勢いです。高いほど引きずる尾が長くなり、激しい攪拌のような跡が残ります。"},paintAmount:{...u.brush,order:3,label:"Paint Amount",min:.2,max:1.2,step:.01,value:d.paintAmount,description:"ペイント時に加える液量です。高いほど厚いインクの塊になり、低いほど薄い膜を重ねる感触になります。"},bubbleForce:{...u.brush,order:4,label:"Bubble Force",min:.05,max:.9,step:.01,value:d.bubbleForce,description:"気泡をくり抜くときの押し広げる勢いです。高いほど周囲の液体を大きく押しのけ、縁に強い波が立ちます。"},bubbleCarve:{...u.brush,order:5,label:"Bubble Carve",min:.2,max:1.2,step:.01,value:d.bubbleCarve,description:"気泡操作でどれだけ液体を削るかです。高いほどはっきり穴が空き、低いと柔らかくえぐるような見え方になります。"},ambientMotion:{...u.brush,order:6,label:"Ambient Motion",min:0,max:.4,step:.01,value:d.ambientMotion,description:"操作していない間のゆるい流れの強さです。上げるほど表面に常時うねりが残り、下げると静かなトレイになります。"},surfaceHeight:{...u.surface,order:1,label:"Surface Height",min:.06,max:.28,step:.005,value:l.surfaceHeight,description:"液体を厚みとして持ち上げる量です。大きいほど膜の輪郭が立体的になり、インクが盛り上がって見えます。"},surfaceRelief:{...u.surface,order:2,label:"Surface Relief",min:0,max:.12,step:.002,value:l.surfaceRelief,description:"表面の細かな凹凸量です。上げると反射が割れ、薄膜のうねりや粘る質感が強調されます。"},meniscus:{...u.surface,order:3,label:"Meniscus",min:0,max:.14,step:.002,value:l.meniscus,description:"液体の縁が立ち上がる量です。上げるほど端で盛り上がり、容器内の濡れた膜らしさが増します。"},absorption:{...u.optics,order:1,label:"Absorption",min:.4,max:3.2,step:.05,value:l.absorption,description:"液体内部で光を吸収する強さです。大きいほど中心が濃く暗くなり、深い色味と厚みが出ます。"},refraction:{...u.optics,order:2,label:"Refraction",min:0,max:.45,step:.01,value:l.refraction,description:"表面越しに見える模様の歪み量です。上げると下層や反射が揺れて、液体らしい屈折感が強まります。"},fresnel:{...u.optics,order:3,label:"Fresnel",min:0,max:1.8,step:.05,value:l.fresnel,description:"視線角で反射が増える量です。上げると斜めから見た縁が明るくなり、濡れた薄膜の存在感が増します。"},specular:{...u.optics,order:4,label:"Specular",min:.2,max:2,step:.05,value:l.specular,description:"ハイライトの鋭さと強さです。上げると濡れたインク表面の照り返しが強くなります。"},caustics:{...u.optics,order:5,label:"Caustics",min:0,max:1,step:.02,value:l.caustics,description:"液体の下側に落ちる淡い光だまりの強さです。上げるとトレイ底面への色移りと厚み感が増します。"},exposure:{...u.optics,order:6,label:"Exposure",min:.7,max:1.4,step:.01,value:T.toneMappingExposure,description:"画面全体の明るさです。上げるとハイライトや透明感が目立ち、下げると色の深さとコントラストが強まります。"},fogDensity:{...u.optics,order:7,label:"Fog Density",min:.02,max:.1,step:.001,value:x.fog.density,description:"背景側の霞みの量です。上げるほど奥行きが強調され、下げるとトレイ全体がくっきり見えます。"}},mt,{title:"Ink Controls",accent:"#9ea8ff",helpText:"スライダーへカーソルを重ねると、シミュレーションと見た目にどのような影響を与えるかを日本語で表示します。"});const ht=document.getElementById("palette"),Te=document.getElementById("toolToggle"),Re=document.getElementById("viewToggle"),vt=document.getElementById("resetButton");se.forEach(t=>{const e=document.createElement("button");e.className="swatch",e.title=t.label,e.dataset.palette=t.id,e.style.background=`rgb(${t.ink.map(n=>Math.round(n*255)).join(" ")})`,e.addEventListener("click",()=>Se(t.id)),ht.appendChild(e)});function Se(t){const e=se.find(n=>n.id===t);e&&(Ee=e,pt(e),document.querySelectorAll("[data-palette]").forEach(n=>{n.classList.toggle("active",n.dataset.palette===t)}))}function C(t){q=t,t==="sheet"&&_.setSceneDamBreak(),t==="drop"&&_.setSceneDroplet(),t==="bubble"&&_.setSceneBubble(),document.querySelectorAll("[data-scene]").forEach(e=>{e.classList.toggle("active",e.dataset.scene===t)})}function le(){Te.textContent=`Tool: ${V==="paint"?"Paint":"Bubble"}`,Re.textContent=`View: ${L==="ink"?"Ink":"Velocity"}`}function Ae(){V=V==="paint"?"bubble":"paint",le()}function Ce(){L=L==="ink"?"velocity":"ink",ae(),le()}Se("lagoon");C("sheet");ae();le();document.querySelectorAll("[data-scene]").forEach(t=>{t.addEventListener("click",()=>C(t.dataset.scene))});Te.addEventListener("click",Ae);Re.addEventListener("click",Ce);vt.addEventListener("click",()=>C(q));const ce=new Ze,Q=new K;function Fe(t,e){const n=g.getBoundingClientRect();Q.x=(t-n.left)/n.width*2-1,Q.y=-((e-n.top)/n.height)*2+1,ce.setFromCamera(Q,A);const i=ce.intersectObject(X,!1)[0];if(!i)return null;const o=X.worldToLocal(i.point.clone());return{x:D.clamp((o.x/p+.5)*M,0,M),y:D.clamp((-o.z/p+.5)*F,0,F)}}const a={mode:null,pointerId:null,previous:{x:0,y:0},simPoint:null,velocity:new K};g.addEventListener("contextmenu",t=>t.preventDefault());g.addEventListener("pointerdown",t=>{if(t.pointerType==="mouse"&&t.button===2){g.setPointerCapture(t.pointerId),a.mode="orbit",a.pointerId=t.pointerId,a.previous={x:t.clientX,y:t.clientY};return}if(t.pointerType==="mouse"&&t.button!==0)return;const e=Fe(t.clientX,t.clientY);e&&(g.setPointerCapture(t.pointerId),a.mode="paint",a.pointerId=t.pointerId,a.previous={x:t.clientX,y:t.clientY},a.simPoint=e,a.velocity.set(0,0))});function Me(t){a.pointerId!==null&&t?.pointerId===a.pointerId&&g.hasPointerCapture(t.pointerId)&&g.releasePointerCapture(t.pointerId),a.mode=null,a.pointerId=null,a.simPoint=null}g.addEventListener("pointerup",Me);g.addEventListener("pointercancel",Me);g.addEventListener("pointerleave",()=>{a.mode=null,a.pointerId=null,a.simPoint=null});g.addEventListener("pointermove",t=>{if(a.mode==="orbit"){h.theta=D.clamp(h.theta-(t.clientX-a.previous.x)*.005,-1.15,1.15),h.phi=D.clamp(h.phi+(t.clientY-a.previous.y)*.005,.55,1.42),a.previous={x:t.clientX,y:t.clientY},te();return}if(a.mode!=="paint")return;const e=Fe(t.clientX,t.clientY);!e||!a.simPoint||(a.velocity.set((e.x-a.simPoint.x)*5.6,(e.y-a.simPoint.y)*5.6),a.simPoint=e)});g.addEventListener("wheel",t=>{t.preventDefault(),h.radius=D.clamp(h.radius+t.deltaY*.01,7.2,15),te()},{passive:!1});window.addEventListener("keydown",t=>{t.key==="1"&&C("sheet"),t.key==="2"&&C("drop"),t.key==="3"&&C("bubble"),(t.key==="b"||t.key==="B")&&Ae(),(t.key==="v"||t.key==="V")&&Ce(),(t.key==="r"||t.key==="R")&&C(q)});H.disable(H.DEPTH_TEST);function _t(){if(!a.simPoint)return;const t=V==="bubble";_.splat(a.simPoint.x,a.simPoint.y,a.velocity.x,a.velocity.y,{radius:ne,velocityAmount:t?d.bubbleForce:d.paintForce,fluidAmount:t?d.bubbleCarve:d.paintAmount,erase:t}),a.velocity.multiplyScalar(.82)}function gt(){if(q!=="sheet"||d.ambientMotion<=0)return;const t=F*.58+Math.sin(S*.9)*F*.02;_.splat(M*.14,t,.65,0,{radius:ne*1.8,velocityAmount:d.ambientMotion,fluidAmount:0})}function xt(){w.rotation.y=Math.sin(S*.18)*.04,W.rotation.z=Math.sin(S*.26)*.08,I.position.x=Math.sin(S*.4)*3.6,I.position.z=Math.cos(S*.4)*3}const bt=2;function Pe(){requestAnimationFrame(Pe),S+=.016,a.mode==="paint"?_t():gt(),$.beginSim();for(let t=0;t<bt;t++)_.step();$.endSim(),_.render(L,Ee,S),R.needsUpdate=!0,f.u_time.value=S,xt(),T.render(x,A),$.update()}window.addEventListener("resize",()=>{A.aspect=window.innerWidth/window.innerHeight,A.updateProjectionMatrix(),T.setSize(window.innerWidth,window.innerHeight)});Pe();
