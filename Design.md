...
- Header navigation update (2026-06-13): the shared main-site header uses `Top`, `Games`, `Webアプリ`, `Moto Catalog`, `Blog`, and `Contact`. Do not expose `Lab` as a top-level header item, and do not expose `3D Texture Reviewer` as a standalone top-level header link.
- The `Webアプリ` dropdown contains `モデる？3Dビューア`, `サムねいる？サムネ作成ツール`, and `モザいく？モザイク加工ツール` as direct service links.
